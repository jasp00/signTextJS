...
function selectCert(userCerts, text, hostname) {
  ...
  for (let cert of userCerts) {
    ...
    sandboxCert.displayName = getCertDisplayName(cert);
...
function signText(context) {
  ...
  try {
    ...
    for (;;) {
      ...
      log("using '" + getCertDisplayName(selectedCert.cert) + "'");
      ...
      if (!autoSign) {
        ...
        if (status != SECSuccess || (cLogin > 0 && cLogin == cBadPwd)) {
          log("Authentication for '" + getCertDisplayName(selectedCert.cert) +
              "' failed");
          cleanupSignTextResources(cert, null, slotList);
          cert = null;
          slotList = null;
